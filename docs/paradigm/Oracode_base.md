...
Accanto all'«Audit» va resa concreta anche la parte «Valore», e qui il paradigma prende posizione su che cosa significhi, in generale, produrre. La produzione di software è produzione nel senso pieno del termine; e la produzione, per essere definita tale, deve poter essere contata. Se l'ammontare prodotto non si può contare, non si sta parlando di produzione ma di attività: righe scritte, ore passate, commit accumulati — il fare, non il prodotto. Da questo discende una scelta che attraversa tutto Oracode: **si produce software conteggiabile**, cioè software costruito in una forma che è enumerabile per costruzione, così che l'ammontare prodotto possa essere riportato su grafici statistici e governato come si governa qualsiasi produzione reale.

Che cosa rende conteggiabile un software? Il software ordinario è una massa continua: righe, file e funzioni che sfumano l'una nell'altra. Di una massa si misura il volume, ma non si conta nulla, perché non ci sono unità. Il cemento colato si misura; i mattoni si contano. Software conteggiabile è software costruito a mattoni, e il mattone è la capacità. Perché sia un'unità di conto vera, ogni capacità deve avere tre proprietà. È **nominata**: una responsabilità con un nome, e il nome è il confine dell'unità — senza nome non c'è unità, c'è solo codice. È **distinta**: mai la stessa responsabilità due volte, perché si contano soltanto elementi discriminabili, e contare due volte lo stesso mattone è truccare la statistica. È **certificata**: la chiusura standard della mission che l'ha consegnata prova che il mattone è reale — si contano cose fatte, non dichiarazioni. Un'unità con queste tre proprietà conta uno. La somma delle unità è l'ammontare prodotto: il grafico non misura il software dall'esterno — il software nasce già in forma di conteggio.

L'unità di conto è semantica, non sintattica: si conta la responsabilità assolta, non le righe o i file che la implementano. Per questo il conteggio è indipendente dal linguaggio di produzione — vale identico per qualsiasi stack, presente o futuro. E per questo la conteggiabilità è un principio di progettazione del codice, non un rilevamento a posteriori: il codice va organizzato perché le sue unità di responsabilità siano nominabili, distinte e certificabili. È la Single Responsibility di SOLID portata al livello della capacità, con un requisito in più che SOLID non conosce: che l'insieme delle responsabilità sia enumerabile. Il nome candidato di questa estensione è **SOLID-C**, dove la C sta per «Conteggiabile». Oggi ogni capacità vale uno; non è un vincolo eterno — se l'uso mostrerà che alcuni tipi di capacità hanno valore intrinseco maggiore, il peso verrà rivisto. Ma il principio non cambia: ciò che non si può contare non è produzione.

La definizione va precisata su un punto dove l'errore è facile e già osservato: la capacità è **comportamento del sistema, mai pezzo tecnico**. Una tabella di database, un model, un controller, un endpoint, un componente di interfaccia non sono capacità: sono componenti d'implementazione — il corpo, non l'unità. La capacità sta al piano di ciò che il sistema sa fare: *«l'amministratore può sospendere una credenziale specificando una motivazione, e il sistema conserva la storia dell'operazione»* è una capacità, anche se realizzarla richiede migration, model, service, controller, interfaccia e test — resta **una** unità funzionale. Nella forma piena: **capacità = unità coesa, funzionale e verificabile di comportamento del sistema, realizzabile mediante riuso, composizione, adattamento o nuova produzione di codice, prodotta nell'ambito di una mission.** (Dettato del CEO, 2026-07-19, M-OS3-160.)

Da quella stessa dettatura discendono tre corollari. Primo: una capacità è **conclusa** solo se soddisfa la sua lista di completezza — scopo unico; criteri di accettazione superati; test adeguati; nessuna capacità esistente rotta; dipendenze dichiarate; documentata; audit superato; commit identificabile (la *Definition of Capability Done*). Secondo: la capacità **non coincide con il codice da scrivere** — descrive ciò che il sistema deve saper fare, e l'analisi stabilisce poi *come* ottenerlo, dichiarando per ogni capacità la **strategia di realizzazione**: riuso, estensione, composizione, rifattorizzazione, o creazione — e quando un asset esistente viene trovato ma deliberatamente scartato, il rifiuto va motivato, perché il riuso non è religione e il codice vecchio sa travestirsi da occasione mentre prepara un'imboscata. Il codice trovato durante l'analisi è **asset candidato** alla realizzazione, mai la capacità stessa: la capacità è il comportamento, l'asset è ciò che può servirlo. Terzo: **il nuovo codice è l'ultima opzione, non la prima** — il valore non sta nella quantità prodotta ma nella quantità di comportamento affidabile che il sistema acquisisce, e per questo si misura anche il non-scritto: una mission che consegna cinque capacità con cento righe può essere più produttiva di una che ne scrive cinquemila. La strategia è una dimensione del record, mai un peso: ogni capacità vale uno per qualunque via sia stata realizzata.
