...
>
> **REGOLA DEL FILE.** Qui vivono le **definizioni** (cosa sono i ruoli, come si nominano, come si relazionano). Lo **STATO corrente** (chi è chi oggi, cosa è fatto, quali clienti esistono) vive in **`NEXUS_HIERARCHY_CURRENT_STATE.md`** e non si duplica: si rimanda. Se questo file e quello divergono sullo stato, vince quello.
>
> **I 5 errori di v2 che v3 corregge ovunque:** (1) 4 livelli → **6 ruoli**; (2) OS3 Matrix «interno a Oracode» → **attuazione al livello Softwarehouse, NON Oracode**; (3) doc-sync **v2** → **v3** (v2 demolita, M-OS3-142); (4) «FlorenceEGI = un livello / caso unico» → **FlorenceEGI = Organismo**, Florence EGI S.R.L. = Softwarehouse, N clienti reali; (5) **accoppiamento al mondo EGI** → il paradigma è **agnostico**: FlorenceEGI è *un esempio* di Organismo, non la definizione. Ogni concetto vale per qualsiasi Organismo/Progetto.
...
## 1. I sei ruoli — definizioni operative

> **Correzione v3 (ratifica CEO 2026-07-11, M-OS3-144).** v2 elencava *quattro livelli* (Oracode / Libreria LSO / Organismo / FlorenceEGI). È superato: i ruoli della gerarchia sono **sei**, e si nominano con **parole, mai con numeri** (le sigle L1/L2/L3 sono in pensione, restano solo tra parentesi come riferimento storico). FlorenceEGI **non è un ruolo**: è *un esempio* di Organismo. La software house acquirente è **Florence EGI S.R.L.**, un ruolo distinto dal suo prodotto FlorenceEGI.

**Una nota sul termine "LSO" prima di iniziare.** Quando si avvia un lavoro applicando Oracode, si crea un'**istanza LSO** (Living Software Organism): *il prodotto vivente di Oracode applicato a un dominio*. Un LSO può essere **mono-organo** (una sola applicazione — ruolo **Progetto**) o **multi-organo** (più applicazioni interconnesse, ciascuna costruita con Oracode — ruolo **Organismo**, i cui membri sono **Organi**); la distinzione è proprietà dell'istanza, non del termine. "LSO" attraversa quindi i ruoli-istanza (Progetto, Organismo, Organo) e dà il nome alla **Libreria LSO** (la collezione di componenti estratti da LSO concreti maturati al punto di essere riusabili).

**I sei ruoli (metro unico, verbatim dal doc di stato corrente `NEXUS_HIERARCHY_CURRENT_STATE.md`).** Le definizioni vivono qui; lo stato (chi-è-chi oggi) vive lì.

| Ruolo | Cos'è | Esempio (uno tra tanti, NON la definizione) |
|---|---|---|
| **Paradigma** *(ex L1)* | Le **REGOLE**, la legge: `oracode` (MIT). Solo metodologia. NON contiene software di prodotto. | — (è unico) |
| **Softwarehouse** *(ex L2)* | L'**acquirente con licenza Nexus** (*Magicsoft 2.0*), SOPRA tutti i clienti. A questo livello vivono le **Librerie LSO** e l'**attuazione del paradigma** — `os3-matrix` (enforcement: librerie, imbrigliamento, hook) + `~/oracode-engine` (motore). Tiene l'HUB: statistiche + numerazione globale su tutta la produzione. | Florence EGI S.R.L. |
| **Libreria LSO** | Repo di **proprietà della software house**, al servizio di tutti i lavori, nessun cliente committente. Il ruolo si dichiara alla creazione via `/project`. | DeepDebug, Fucina, Cockpit, EGI-STAT, SNC |
| **Progetto** *(ex L3 mono-organo)* | **Un LSO mono-organo**: sta per conto suo. | Capasso |
| **Organismo** *(ex L3-hub)* | **Un LSO multi-organo**, col suo **repo-centro** (`ssot_home` = sé; gli Organi gli puntano). | FlorenceEGI (repo-centro: EGI-DOC) |
| **Organo** | **Un LSO appartenente a un Organismo** (`ssot_home` = il repo-centro dell'Organismo). | EGI, EGI-HUB, EGI-Credential |

Le sei definizioni che seguono (§1.1-§1.6) approfondiscono ciascun ruolo; il termine "LSO" è il nome operativo che lega i ruoli-istanza all'esperienza concreta di costruire e far vivere un sistema Oracode.

**Asse dei ruoli vs asse operativo (Oracode Nexus).** I sei ruoli dicono *chi possiede cosa e a chi serve*. Sono ortogonali all'asse **operativo di deployment** definito in `ORACODE_NEXUS_3_TIER.md` (le cui definizioni strumento/softwarehouse/istanza restano **valide e LOCKED**, si nominano solo coi ruoli) e all'asse di **maturità** (il campo `level`/`maturity_level` del descrittore, valori 1-4 — un altro metro ancora). Tre assi distinti: **ruolo** (chi), **deployment** (dove vive lo stato), **maturità** (quanto è evoluto). Confonderli è la fonte principale dei "casini": vanno tenuti separati.
...
**Oracode è SOLO il paradigma.** Oracode è la metodologia — la legge — e nient'altro: regole epistemiche, pilastri, triade OSZ/OS3/OS4, pattern architetturali (Mission Protocol, DOC-SYNC, RAG, Helping), standard (SEO, A11y, GDPR), disciplina documentale, Strategia Delta, layer stack L0-L11. Tutto ciò che è trasferibile a qualsiasi dominio come metodologia. Oracode **non contiene software di prodotto**.

**OS3 Matrix non è Oracode.** L'enforcement (hook bloccanti, gate validator, agenti specializzati, **DOC-SYNC v3** runtime, drift detection, SSOT registry, Mission Protocol automatico) è il software che *fa rispettare* le regole del paradigma al piano OS3, dove l'AI opera come co-autore a velocità non verificabile dall'umano in tempo reale. Ma è **l'attuazione del paradigma**, di proprietà della **software house** (ruolo *Softwarehouse* — vedi la tabella dei 6 ruoli in §1), nella stessa classe delle Librerie LSO. NON è un piano interno di Oracode.

Il rapporto è di servizio, non di appartenenza: **il paradigma prescrive, OS3 Matrix enforcea**. Senza il paradigma, la Matrix non ha regole da far rispettare; senza la Matrix, le regole restano indicazioni interpretabili — fragili sotto la pressione di un LLM che produce a 100x la velocità dell'umano. Ma sono **due ruoli distinti** (Paradigma vs Softwarehouse), non due facce di uno stesso Oracode.

> **⛔ Correzione M-OS3-147.** v2 qui diceva «Oracode si articola formalmente in due piani interni: §1.1.A Oracode-paradigma + §1.1.B OS3 Matrix» (framing dalla proposta `M-NOMENCL-OSMX-002`, **mai ratificata dal CEO**). È superato: OS3 Matrix sta al livello **Softwarehouse**, non dentro Oracode. Le due sotto-sezioni che seguono sono mantenute come **descrizione tecnica** — §1.1.A la composizione del *paradigma*, §1.1.B la descrizione di *OS3 Matrix* — ma §1.1.B va letta come **prodotto della software house, NON articolazione di Oracode**.
...
#### 1.1.B — OS3 Matrix (OSMx): l'attuazione al livello Softwarehouse (NON Oracode)

> Sezione mantenuta per stabilità dei riferimenti (`§1.1.B` è citato altrove), ma **OS3 Matrix non appartiene a Oracode**: è l'attuazione del paradigma, ruolo **Softwarehouse**, stessa classe delle Librerie LSO (§1, correzione M-OS3-147).
...
##### 3.1.A.6 Primitivi OSZ (livello kernel) — rimando alla fonte

I **primitivi del kernel** — le unità semantiche fondamentali (`Wrapper<T>`, `Interface`, `Nerve`, la pipeline semantica, la metrica di affidabilità) — **non si definiscono qui**. Vivono nel kernel: `docs/paradigm/kernel/00_OSZ_ORACODE_SYSTEM_ZERO.md`, sezione «I Primitivi di OSZ». La nomenclatura **rimanda** e non li duplica: una sola SSOT per responsabilità, altrimenti le due copie driftano (era il caso di v2, che ricopiava la tabella).

> **⚠️ Accoppiamento kernel da sciogliere — segnalato da M-OS3-147, disaccoppiamento in mission kernel dedicata (classe dura, merge CEO).** Il kernel oggi nomina un primitivo **`EGI`** (`Wrapper<T> + Regole + Audit + Valore`, `00_OSZ:71`). Ma "EGI" è branding dell'**Organismo** FlorenceEGI, non un nome universale del paradigma. La **forma universale** del primitivo è *«unità di valore tracciata, autenticata e auditabile»*: è QUELLA il primitivo del paradigma; "EGI" ne è l'**istanza** in FlorenceEGI. Rinominare il primitivo all'universale tocca il kernel (classe dura) ed è **fuori dallo scope della nomenclatura**. Finché non è fatto: leggi «EGI (primitivo)» come «unità-di-valore universale (istanza FlorenceEGI: EGI)». Stesso discorso per `USE`/`URS` se, verificando il kernel, risultano artefatti d'istanza promossi a kernel.
