---
title: ORACODE PARADIGM
slug: oracode-paradigm-v2-draft
doc_type: concept
version: 1.0.0
status: current
date: '2026-05-29'
...
Il flusso operativo canonico di Oracode Nexus si articola in tre comandi sequenziali: **`/discovery`** (acquisizione pre-sales del nuovo progetto: si capisce cosa il committente vuole, si qualifica il dominio) → **`/project`** (bootstrap dell'istanza) → **`/mission`** (il lavoro di sviluppo vero e proprio). Questa sezione descrive `/project`, il momento in cui un nuovo progetto Oracode viene concepito, prima ancora che la prima riga di codice sia scritta.
...
- Si scaffolda il descrittore `.oracode/project.json` dell'istanza — il file che lega il progetto al suo MISSION_REGISTRY di Livello 3 e abilita il ponte automatico L1→L3 (la state machine dell'engine propaga ogni transizione al registry del repo, vedi Macroarea quattro, sezione 10)
- Si pianificano le prime mission di sviluppo

`/project` si chiude quando il boot context è pronto, il descrittore `.oracode/project.json` è in posto, e la prima mission può aprirsi su basi solide. Solo a quel punto il codice comincia a essere scritto.
...
Dopo `/project` viene `/mission`, l'unità discreta di lavoro Oracode. Ogni cambiamento al codice — sia esso una feature nuova, un bug fix, un refactor, una migrazione legacy — passa attraverso una mission. `/mission` è un comando **globale e context-aware**: vive a livello dell'engine (Livello 1), rileva l'istanza corrente dal CWD risolvendo il descrittore `.oracode/project.json`, e auto-registra la mission nel MISSION_REGISTRY (Livello 3) del progetto. Non è dunque un sottocomando vincolato a stare "dentro" un singolo `/project`: è lo stesso comando per tutte le istanze della macchina, che capisce da solo su quale progetto sta operando.
...
**Nota di disambiguazione.** I quattro livelli di applicazione descritti qui non vanno confusi con i quattro livelli concettuali definiti nella Nomenclatura LSO v2.0.0 (Oracode / Libreria LSO / Organismo / istanza esemplare, es. FlorenceEGI). Quelli sono livelli *concettuali*: separano cosa Oracode *è* come entità. Questi sono livelli *applicativi*: descrivono a quale profondità Oracode viene *applicato* a un progetto. Le due tassonomie sono ortogonali e complementari: la prima risponde a "di che tipo di cosa stiamo parlando?", la seconda a "quanto in profondità è stato applicato il paradigma?".

Esiste inoltre una **terza** tassonomia, ortogonale a entrambe: la gerarchia operativa a 3 livelli di **Oracode Nexus** — **L1 GLOBALE** (motore + paradigma, `~/oracode-engine` + `os3-matrix`), **L2 HUB** (softwarehouse acquirente, con il registry consolidato e le statistiche cross-istanza), **L3 ISTANZA** (singolo progetto/cliente, col proprio MISSION_REGISTRY). Questa descrive *dove vivono motore, registri e statistiche nell'ecosistema*, non *cosa Oracode è* né *quanto in profondità è applicato*. Con "Oracode Nexus" si intende il sistema completo: il paradigma più i suoi tre livelli operativi più l'ecosistema HUB/istanze. Riferimento: `ORACODE_NEXUS_3_TIER.md`.
...
**Libreria hook.** Una collezione di script eseguibili (PreToolUse, PostToolUse, PreUserPromptSubmit) che intercettano le azioni dell'LLM e le validano contro le regole. Ogni hook è file dedicato a una verifica specifica: un hook per P0-4 (esistenza metodi), uno per P0-6 (service-method), uno per regole dominio-specifiche (es. normative crypto MiCA), eccetera. La libreria è estensibile — nuovi hook si aggiungono per nuove regole.
...
**Mission Registry.** Strumento di gestione del ciclo di vita delle mission. Contiene il file `MISSION_REGISTRY.json` (o equivalente per stack diversi) che traccia tutte le mission aperte e chiuse del progetto, con i loro metadati. Le chiavi canoniche sono in **inglese** (decisione CEO): `id`, `title`, `type`, `organs`, `status`, `date_open`, `date_close`, `report_technical`, `stats` — l'italiano (`tipo_missione`, `organi_coinvolti`, `data_apertura`, `stato`) è legacy (es. istanza FlorenceEGI: EGI-DOC), mai canonico per istanze nuove.

In Oracode Nexus il MISSION_REGISTRY esiste su **due** livelli, da non confondere: **L3 — istanza/progetto** (nel repo del progetto, è il proprio registry, schema chiavi inglesi sopra) e **L2 — HUB softwarehouse** (il file unico consolidato, con numerazione globale unica e statistiche cross-istanza calcolate aggregando le decine di cartelle delle istanze). Il **primo vero registry** consolidato è L2. Il **L1 — motore** (`~/oracode-engine`) NON è un registro: tiene solo lo scratch runtime (lock, focus, stato in volo), non un archivio versionato. Il registry di progetto (L3) è la fonte di verità sullo stato del lavoro *dell'istanza*; le statistiche aggregate vivono a L2. Riferimento: `ORACODE_NEXUS_3_TIER.md`.

**Mission Engine dual-tracking.** Dal bootstrap di OS3 Matrix (M-OS3-001, 25 maggio 2026) il Mission Registry vive in due fonti dati distinte. La prima è la **state machine non-aggirabile** della CLI `bin/mission`, che vive in `~/oracode-engine/missions/<ID>/state.json` (la cartella globale visibile dell'engine — Livello 1 di Oracode Nexus; `~/.oracode` resta come symlink di compat), condivisa fra tutti i progetti Oracode della macchina; tiene firma crittografica della catena di transizioni, spawn fingerprint reali degli agenti, contatori di edit per audit interim, eventuale flag di sospensione. La seconda è il **Mission Registry del progetto applicativo**, versionato col repo (`<progetto-DOC>/docs/missions/MISSION_REGISTRY.json`), che traccia scope dichiarato, trigger matrix, SSOT impattati, P0 di dominio, esito test, blocco `doc_sync` prodotto da DOC-SYNC v2 alla chiusura. La separazione corrisponde a una distinzione concettuale netta: la state machine è infrastruttura di disciplina (universale, parte di OS3 Matrix, Livello 1), il Mission Registry del progetto è SSOT documentale (specifico al dominio, parte dell'organo, Livello 3). La propagazione fra la state machine (L1) e il Mission Registry del progetto (L3) è **automatica**: `bin/mission` legge il descrittore `.oracode/project.json` del progetto attivo (risolto dal CWD) e propaga ogni transizione al `MISSION_REGISTRY.json` del repo, parallel-safe con lockfile per-registry (funzione `syncToRepoRegistry`). La vecchia sincronizzazione manuale via Edit dedicato (debito tecnico drift S2-1, sessione 2 Poli) è **risolta**: il ponte L1→L3 è implementato e attivo.
