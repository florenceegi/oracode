...
### 8.3 Indipendenza dall'istanza LSO

Il CLI è **Oracode-universale**: nessun riferimento hardcoded a path di un'istanza specifica. Il registro mission risiede in path relativo all'installazione dell'istanza (es. `EGI-DOC/docs/missions/` per FlorenceEGI, `Poli-Doc/docs/missions/` per Poli).
...
**Soluzione v0.3.** Focus per-session via environment variable. Ogni sessione AI agent ha env univoca (in Claude Code: `CLAUDE_CODE_SESSION_ID`). Il focus è scritto in `~/.oracode/focus/<session_id>.json`. N sessioni = N focus paralleli, zero collisione.

**Fallback terminale standalone.** Quando l'env non è presente (terminale interattivo umano senza sessione AI), si usa il file legacy `focus.json` + `active-mission.lock`.
